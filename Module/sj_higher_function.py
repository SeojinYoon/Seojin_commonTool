...
def recursive_mapWithDepth(x, function, apply_depth):
    ...
    :param x: data ex) [[1,2]]
    :param function: function ex) lambda x: lambda x: round(x,1)
    :param apply_depth: depth(int) / depth value of specific iterable data to apply function
...
class Mapping:
    ...
    def one_to_many(domain, codomain, mapping_condition):
        ...
        :param codomain(list): codomain
        :param mapping_condition: mapping condition(function - argument count is 2)

        :return (2d list): mapped list, domain to codomain
        """
...
def get_function_args(function_name, module_name = "__main__"):
    ...
    :param module_name(string): module name ex) sys.modules[__name__]
    :param function_name(string): function name ex) clean
    
    return (list): arg names of function
    """
...
def multi_mapping(func_name, arg_value_pairs, module_name = "__main__"):
    ...
    :param function_name(string): function name ex) "clean"
    :param arg_value_pairs(list): arguments ex) [[1, 2], [3, 4]]
    :param module_name(string): module name ex) sys.modules[__name__]
...
def call_func_dynamically(function_name, argument_names, arg_value_pair, module_name = "__main__"):
    ...
    :param argument_names(list): arg names ex) ["a", "b"]
    :param arg_value_pair(list): arg value pair, corresponding arg_name ex) [1, 2] <- number or ['1', '2'] <- string
    :param module_name(string): module name, ex) sys.modules[__name__]
...
def delayed_apply(func, arg_info, module_name = "__main__"):
    """
    Do not use in parallel processing!!!!
    
    delayed apply function
    
    ex) delayed_apply(lambda s, b: s + b, {"s": 1})({"b": 4})()
    
    :param func: function
    :param arg_info(dict): func argument info
    """
    func_name = func.__name__
    func_arg_spec = sorted(inspect.getargspec(func)[0])
    
    module = sys.modules[module_name]
    
    if func_arg_spec == sorted(list(arg_info.keys())):
        return func(**arg_info)
    else:
        # Save context
        func_key = hashlib.sha256(bytes(func_name, 'utf-8')).hexdigest() + "_func"
        func_args_key = hashlib.sha256(bytes(func_name, 'utf-8')).hexdigest() + "_args"
        
        setattr(module, func_key, func)
        setattr(module, func_args_key, arg_info)
        
        def accumuluate_args(arg_info2 = None):
            """
            Accumuluate arguments
            
            :param arg_info(dict): 
            """
            # Get context
            func = getattr(module, func_key)
            args = getattr(module, func_args_key)
            
            if arg_info2 != None:
                args.update(arg_info2)
                    
            if func_arg_spec == sorted(list(args.keys())):
...
if __name__ == "__main__":
    ...
    delayed_apply(lambda s, b: s + b, {"s": 1})({"b": 4})()
